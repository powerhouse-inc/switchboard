...
export const fetchOrThrow = async <T>(builderOutput: { query: string, variables: any }) => {
  const operationName = (parse(builderOutput.query).definitions[0] as any).selectionSet.selections[0].name.value;
  const result = await ctx.client.request(builderOutput.query, builderOutput.variables) as any;
